...
import { X, Type, Loader2, Sparkles } from 'lucide-react';
...
const STYLE_PRESETS = [
    {
        ...
        emoji: '🎵',
        config: {
            fontName: 'Impact', fontSize: 52, fontColor: '#FFFFFF', highlightColor: '#FFDD00',
            borderColor: '#000000', borderWidth: 4, bgColor: '#000000', bgOpacity: 0,
            animation: 'pop', uppercase: true, maxCharsPerBlock: 12, verticalPosition: 82,
        },
    },
    {
        label: 'Instagram',
        emoji: '📸',
        config: {
            fontName: 'Verdana', fontSize: 42, fontColor: '#FFFFFF', highlightColor: '#FF69B4',
            borderColor: '#000000', borderWidth: 3, bgColor: '#000000', bgOpacity: 0,
            animation: 'bounce', uppercase: false, maxCharsPerBlock: 16, verticalPosition: 80,
        },
    },
    {
        label: 'Karaoke',
        emoji: '🎤',
        config: {
            fontName: 'Arial', fontSize: 44, fontColor: '#FFFFFF', highlightColor: '#00FF88',
            borderColor: '#000000', borderWidth: 3, bgColor: '#000000', bgOpacity: 0,
            animation: 'karaoke', uppercase: false, maxCharsPerBlock: 18, verticalPosition: 80,
        },
    },
    {
        label: 'Box',
        emoji: '📦',
        config: {
            fontName: 'Verdana', fontSize: 36, fontColor: '#FFFFFF', highlightColor: '#FFDD00',
            borderColor: '#000000', borderWidth: 0, bgColor: '#000000', bgOpacity: 0.7,
            animation: 'pop', uppercase: false, maxCharsPerBlock: 20, verticalPosition: 80,
        },
    },
];
...
export default function SubtitleModal({ isOpen, onClose, onGenerate, isProcessing, videoUrl, jobId, clipIndex, existingHook, existingEffects = null }) {
    ...
    const [maxCharsPerBlock, setMaxCharsPerBlock] = useState(12);
    ...
    const applyPreset = (preset) => {
        ...
        setHighlightColor(c.highlightColor);
        setBorderColor(c.borderColor);
        setBorderWidth(c.borderWidth);
        setBgColor(c.bgColor);
        setBgOpacity(c.bgOpacity);
        setAnimation(c.animation);
        setUppercase(c.uppercase);
        setMaxCharsPerBlock(c.maxCharsPerBlock);
        setVerticalPosition(c.verticalPosition);
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[#121214] border border-white/10 p-6 rounded-2xl w-full max-w-5xl shadow-2xl relative flex flex-col md:flex-row gap-6 max-h-[90vh]">
                ...
                {/* Left: Live Preview */}
                <div className="flex-1 flex flex-col items-center justify-center bg-black rounded-xl border border-white/5 overflow-hidden relative aspect-[9/16] max-h-[600px]">
                    {captionsLoading ? (
                        ...
                        <>
                            ...
                            <div
                                className="absolute left-0 right-0 px-6 text-center transition-all duration-300 pointer-events-none flex flex-col items-center"
                                style={{ top: `${verticalPosition}%`, transform: 'translateY(-50%)' }}
                            >
                                <span style={fallbackPreviewStyle}>
                                    {uppercase ? 'EXAMPLE SUBTITLE' : 'Example Subtitle'}
                                </span>
                            </div>
                        </>
                    )}
                ...
                <div className="w-full md:w-80 flex flex-col overflow-hidden">
                    <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2 shrink-0">
                        <Type className="text-primary" /> Subtitles
                    </h3>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {STYLE_PRESETS.map((preset) => (
                                    <button
                                        ...
                                        className="p-2 rounded-lg border border-white/5 bg-white/5 hover:bg-white/10 text-center text-xs font-bold text-zinc-300 hover:text-white transition-all"
                                    >
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                ...
                                <span className="text-[10px] font-mono text-primary bg-primary/10 px-1.5 py-0.5 rounded">{verticalPosition}%</span>
                            </div>
                            <input
                                type="range"
                                min="5"
                            ...
                            <div className="grid grid-cols-2 gap-1.5">
                                {ANIMATION_OPTIONS.map((opt) => (
                                    <button
                                        ...
                                        className={`p-2 rounded-lg border text-center text-xs font-semibold transition-all ${animation === opt.value ? 'bg-primary/20 border-primary text-white' : 'bg-white/5 border-white/5 text-zinc-400 hover:bg-white/10'}`}
                                    >
                                        {opt.label}
                                    </button>
                                ))}
                        ...
                        <div>
                            ...
                            <input
                                type="range" min="24" max="72" value={fontSize}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Words Per Line</label>
                                <span className="text-[10px] font-mono text-zinc-500">~{Math.ceil(maxCharsPerBlock / 5)} words</span>
                            </div>
                            <input
                                type="range" min="6" max="28" value={maxCharsPerBlock}
                                onChange={(e) => setMaxCharsPerBlock(parseInt(e.target.value))}
                                className="w-full accent-primary"
                            />
                            <div className="flex justify-between text-[10px] text-zinc-600 mt-1">
                                <span>1 word</span>
                        ...
                        <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5">
                            <label className="text-xs font-bold text-zinc-300">UPPERCASE TEXT</label>
                        ...
                        {/* Font */}
                        <div>
                            <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5 block">Font</label>
                            <select
                                value={fontName}
                        ...
                        {/* Text Color */}
                        <div>
                            <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5 block">Text Color</label>
                            <div className="flex flex-wrap gap-2">
                                ...
                                <label className="w-7 h-7 rounded-full border-2 border-dashed border-white/30 cursor-pointer flex items-center justify-center hover:border-white/60 transition-all overflow-hidden relative" title="Custom color">
                        ...
                        {/* Highlight Color */}
                        <div>
                            <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5 block">Highlight Color</label>
                            <div className="flex flex-wrap gap-2">
                                {[
                                ...
                                <label className="w-7 h-7 rounded-full border-2 border-dashed border-white/30 cursor-pointer flex items-center justify-center hover:border-white/60 transition-all overflow-hidden relative">
                                    <span className="text-[10px] text-zinc-400">+</span>
                        ...
                        {/* Border / Outline */}
                        <div>
                            <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5 block">Text Stroke / Border</label>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    <input
                                        type="range" min="0" max="6" value={borderWidth}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" checked={bgOpacity > 0} onChange={(e) => setBgOpacity(e.target.checked ? 0.6 : 0)} className="sr-only peer" />
                                    <div className="w-8 h-4 bg-zinc-700 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[0px] after:left-[0px] after:bg-white after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                                </label>
                            </div>
                            {bgOpacity > 0 && (
                    ...
                    >
                        ...
                        {isProcessing ? 'Rendering...' : 'Apply Subtitles'}
                    </button>
                </div>
            </div>
        </div>
